...
function SectionOrderEditorItem({
    ...
    return (
        ...
        >
            ...
            <span
                className={`flex-1 text-sm ${visible ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400 dark:text-gray-500 line-through'}`}>
        {label}
      </span>
    ...
    useEffect(() => {
        ...
        supabase.auth.getUser().then(({data: {user}}) => {
            ...
            supabase
                ...
                .maybeSingle()
                .then(({data}) => {
                    if (data?.fcm_token) setNativeSubscribed(true);
                });
        });
    }, []);
    ...
    const handleToggleNotifications = async () => {
        if (isNative()) {
            ...
            try {
                ...
                if (!granted) {
                    toast.error(t('settings.notifPermissionDenied'));
                    return;
                }
                const fcmToken = await getNativeFcmToken();
                if (!fcmToken) {
                    toast.error(t('settings.notifTokenError'));
                    return;
                }
                ...
                if (!error) {
                    ...
                    toast.success(t('settings.notifEnabled'));
                } else {
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                                ...
                                <div className="w-full space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={handleSaveName}
                                                className="btn-primary text-sm py-1.5 flex-1">{t('settings.save')}</button>
                                        <button onClick={() => setEditingName(false)}
                                                className="btn-ghost text-sm py-1.5 flex-1">{t('settings.cancel')}</button>
                                    </div>
                                </div>
                            ) : (
                                            ...
                                            <>
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-2.5">
                                                        <Bell size={15}
                                                              className={isSubscribed ? 'text-teal-500' : 'text-gray-400'}/>
                                                        <span
                                        ...
                                        <div
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    <span
                                                        className="text-sm text-gray-700 dark:text-gray-300">{t('settings.notifEpisodes')}</span>
                                                </div>
                                                <Toggle
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    <span
                                                        className="text-sm text-gray-700 dark:text-gray-300">{t('settings.notifSeasons')}</span>
                                                </div>
                                                <Toggle
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    <span
                                                        className="text-sm text-gray-700 dark:text-gray-300">{t('settings.notifMovies')}</span>
                                                </div>
                                                <Toggle
                        ...
                        {(subscribed || nativeSubscribed) && (
                            <div className="mt-3 space-y-2">
                                <button
                                    ...
                                    disabled={notifLoading || nativeLoading || testNotifState === 'loading'}
                    ...
                    <section>
                        ...
                        <div className="card p-4 space-y-2">
                            <div className="flex items-center justify-between">
                                <span
                                    className="text-sm text-gray-600 dark:text-gray-400">{t('settings.version')}</span>
                                <span
                                    className="text-sm font-medium text-gray-900 dark:text-gray-100">{APP_VERSION}</span>
                            </div>
                            <div className="flex items-center justify-between">
            ...
            {dragPreview && (
                ...
                >
                    <div
                        className="flex items-center gap-3 rounded-xl border border-amber-400 bg-[#fff7e8] dark:bg-[#2b2417] px-3 py-2.5 shadow-[0_18px_40px_rgba(15,23,42,0.28)]">
                        <GripVertical size={14} className="text-amber-500 shrink-0"/>
                        <span
                            className={`text-sm ${dragPreview.visible ? 'text-gray-900 dark:text-gray-100' : 'text-gray-400 dark:text-gray-500 line-through'}`}>
              {dragPreview.label}
            </span>
                        {dragPreview.visible ? <Eye size={16} className="text-amber-600 dark:text-amber-400"/> :
