...
export default function BookDetailModal({ book, onClose }: Props) {
  ...
  const handleToggleStatus = async () => {
    ...
    toast.success(`Moved to ${newStatus === 'read' ? 'Read' : 'Want to read'}`);
  };
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <div className="flex flex-wrap gap-2 text-sm">
              ...
              <span className={`px-3 py-1 rounded-full font-medium text-white ${
                ...
                {localBook.status === 'read' ? 'Read' : 'Want to read'}
              </span>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-sm text-gray-500 dark:text-gray-400">Personal note</p>
                {!editingNote && (
              ...
              {editingNote ? (
                ...
                <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                  {localBook.personal_note || <span className="text-gray-400 italic">No notes yet. Click the pencil to add one.</span>}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <button onClick={handleToggleStatus} className="btn-ghost text-sm flex items-center gap-1.5">
                ...
                Move to {localBook.status === 'read' ? 'Want to read' : 'Read'}
              </button>
              {!confirmDelete ? (
