...
export default function CategorySeriesPickerModal({ category, series, onConfirm, onClose }: Props) {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set(category.series_ids));

  const filtered = useMemo(() => {
    const q = query.toLowerCase();
    return series.filter(s =>
      !q || s.title.toLowerCase().includes(q) || s.creator.toLowerCase().includes(q)
    );
  }, [series, query]);
  ...
  const handleConfirm = () => {
    const newIds = [...selected].filter(id => !category.series_ids.includes(id));
    onConfirm(newIds);
  };

  const newCount = [...selected].filter(id => !category.series_ids.includes(id)).length;
    ...
    >
        ...
        <div className="px-5 py-3 flex-shrink-0">
          <div className="relative">
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder={t('seriesLibrary.searchSeries')} className="input pl-9 text-sm py-2" autoFocus />
          </div>
        </div>

        <div className="flex-1 overflow-y-auto px-3 pb-3">
          {filtered.length === 0 ? (
            <p className="text-center text-sm text-gray-400 py-10">{t('seriesLibrary.noSeriesFound')}</p>
          ) : (
            filtered.map(s => {
              const isSelected = selected.has(s.id);
              const alreadyIn = category.series_ids.includes(s.id);
              return (
                <button
                  key={s.id}
                  onClick={() => toggle(s.id)}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{s.creator}</p>
                  </div>
                  {alreadyIn ? (
                    <span className="flex-shrink-0 text-xs text-emerald-600 dark:text-emerald-400 font-medium">
                      {t('seriesLibrary.alreadyInCategory')}
                    </span>
                  ) : (
        ...
        <div className="p-5 border-t border-black/[0.06] dark:border-white/[0.06] flex gap-3 flex-shrink-0">
          <button onClick={onClose} className="btn-ghost flex-1 text-sm">{t('seriesLibrary.cancel')}</button>
          <button onClick={handleConfirm} disabled={newCount === 0} className="btn-primary flex-1 text-sm disabled:opacity-40">
            {newCount > 0 ? t('seriesLibrary.addNSeries', { count: newCount }) : t('seriesLibrary.confirmAdd')}
          </button>
        </div>
    </SheetModal>
  );
