...
export function useNotificationSubscription() {
    ...
    const sendTestNotification = useCallback(async (fcmToken?: string): Promise<PushTestResult> => {
        if (!supported && !fcmToken) {
            return {ok: false, message: 'Notifications non supportées sur cet appareil.'};
        }

        const body = fcmToken ? { token: fcmToken, transport: 'fcm' } : {};
        ...
        if ((result.data?.sent ?? 0) > 0) {
            ...
            return { ok: true, message: 'Notification de test envoyée.' };
        }
